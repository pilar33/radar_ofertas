...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--preset", default="auto", choices=["auto", "tiendanube"])

    def handle(self, *args, **options):
        preset = None if options["preset"] == "auto" else options["preset"]
        resultado = analizar_url_laboratorio(options["url"], limite=options["limite"], modo=options["modo"], preset=preset)
        ...
        self.stdout.write(f"Plataforma: {resultado.get('plataforma_detectada') or '-'}")
        ...
        for item in resultado["productos_detectados"][: options["limite"]]:
            self.stdout.write(
                "- "
                f"{item.get('titulo') or '-'} | "
                f"lista={item.get('precio_lista_decimal')} | "
                f"transferencia={item.get('precio_transferencia_decimal')} | "
                f"tarjeta={item.get('precio_tarjeta_decimal')} | "
                f"oportunidad={item.get('precio_oportunidad_decimal')} "
                f"({item.get('tipo_precio_oportunidad')}) | "
                f"imagen={'si' if item.get('imagen_url') else 'no'} | "
